...
function choosen_y() {
    ...
    const validationResult = fieldValue.match(/^[-]?(((0{1}|[1-4]){1}(\.[0-9]+)?)|5(\.0+)?)$/);
...
function choosen_r() {
    const field = $('#r_field');
    const fieldValue = field.val();
    const validationResult = fieldValue.match(/^(([1-3]{1}(\.[0-9]+)?)|4(\.0+)?)$/);
    ...
    $("#form").submit(function (event) {
        ...
        if (submitValidate()) {
            ...
            $.ajax({
                method: "POST",
                url: "script.php",
                data: {xValue: xVal, yValue: yVal, rValue: rVal}
            }).done(function (jsonResponse) {
                console.log("handle response")
                addToStorage(jsonResponse);
                toHtmlTable(jsonResponse);
            });
        } else {
